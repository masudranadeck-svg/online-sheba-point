...
export default function ShopPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        console.log("প্রোডাক্ট আনতে সমস্যা হয়েছে!");
      }
    };
    fetchProducts();
  }, []);
  ...
  // ডাটাবেসের ফিল্ড অনুযায়ী ফিল্টার করা (category)
  ...
  // কার্টে যোগ করার সময় _id ব্যবহার করা হয়েছে
    ...
    <div style={{minHeight:'100vh'}}>
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'32px 24px'}}>
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:32}}>
          {cats.map(c=>(
            <button key={c.id} onClick={()=>setCat(c.id)} className={cat===c.id?'d-btn glow-btn':''} style={cat!==c.id?{background:'white',color:'#888',border:'2px solid #e8ecf1',borderRadius:12,padding:'8px 20px',fontSize:13,fontWeight:600,cursor:'pointer',transition:'all 0.3s'}:{}}>{c.n}</button>
          ))}
        </div>
          ...
          {items.map(p=>(
            <div key={p._id} className={`d-card ${typeGlow(p.category)}`}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                ...
                <span className="d-tag" style={{background:'#f5f5f5',color:'#888'}}>নতুন</span>
              </div>
              <h3 style={{fontSize:14,fontWeight:700,marginBottom:4,color:'#1a1a2e',margin:'0 0 4px 0'}}>{p.name}</h3>
              <p style={{fontSize:12,color:'#aaa',marginBottom:12,margin:'0 0 12px 0'}}>{p.description}</p>
        ...
        {items.length===0&&<div style={{textAlign:'center',padding:64}}><p style={{fontSize:48,marginBottom:16}}>😔</p><p style={{fontSize:16,color:'#aaa'}}>কোনো প্রোডাক্ট পাওয়া যায়নি</p></div>}
      </div>

      {showCart&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:50,padding:16}} onClick={()=>setShowCart(false)}>
          <div style={{background:'white',borderRadius:20,padding:24,maxWidth:420,width:'100%',maxHeight:'80vh',overflowY:'auto',boxShadow:'0 20px 60px rgba(0,0,0,0.2)'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
              <h2 style={{fontSize:20,fontWeight:700,color:'#1a1a2e',margin:0}}>🛒 কার্ট</h2>
              <button onClick={()=>setShowCart(false)} style={{background:'none',border:'none',fontSize:20,color:'#aaa',cursor:'pointer'}}>✕</button>
            </div>
            {cart.length===0?<div style={{textAlign:'center',padding:32}}><p style={{fontSize:40,marginBottom:8}}>🛒</p><p style={{color:'#aaa'}}>কার্ট খালি!</p></div>:(
              <>
                <div style={{marginBottom:24}}>{cart.map(c=>(<div key={c._id} style={{background:'#f5f7fa',borderRadius:12,padding:16,marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div><h4 style={{fontWeight:600,fontSize:14,color:'#1a1a2e',margin:'0 0 4px 0'}}>{c.name}</h4><p style={{color:'#2dce89',fontWeight:700,fontSize:14,margin:0}}>৳{c.price} × {c.qty}</p></div>
                  <button onClick={()=>removeCart(c._id)} style={{background:'none',border:'none',fontSize:18,cursor:'pointer',color:'#ff6b6b'}}>🗑</button>
                </div>))}</div>
                <div style={{borderTop:'1px solid #f0f0f0',paddingTop:16,marginBottom:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <span style={{fontWeight:700,color:'#1a1a2e'}}>মোট：</span>
                  <span style={{fontSize:24,fontWeight:700,color:'#2dce89'}}>৳{total}</span>
                </div>
                <Link href="/checkout" className="d-btn-green glow-btn-green" style={{display:'block',textAlign:'center',padding:'12px 0',fontSize:14,textDecoration:'none'}} onClick={()=>setShowCart(false)}>💳 চেকআউট</Link>
              </>
            )}
          </div>
        </div>
